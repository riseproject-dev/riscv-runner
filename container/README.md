...
For architecture, sequence diagrams, the database schema, the demand-matching algorithm, the installation event log, and ops runbooks, see the [website](https://riscv-runners.riseproject.dev/). This README covers only what a contributor working in `container/` needs to know.

Go module: `github.com/riseproject-dev/riscv-runner/container`.
...
CI mirrors this in [`.github/workflows/deploy-container.yml`](../.github/workflows/deploy-container.yml) before building images.
...
Deploy runs through [`.github/workflows/deploy-container.yml`](../.github/workflows/deploy-container.yml). Push to `main` pushes both images to the Scaleway registry, runs `npx serverless deploy --stage=staging`, triggers the staging sample workflow as a smoke test, then waits for an environment-gated approval before deploying to prod. Manual deploys are available via `workflow_dispatch`.
