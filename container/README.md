...
The Go module is `github.com/riseproject-dev/riscv-runner/container`.
...
The app uses a **demand-matching** model: on one side, `workflow_job` webhooks create demand for runners; on the other, Kubernetes workers provide supply. The scheduler scales supply to match demand per `(entity, job_labels)` pool, with configurable limits.
...
Each runner pod is a single container running the image built from [`../images/runner/Dockerfile.ubuntu`](../images/runner/Dockerfile.ubuntu). The pod runs `privileged: true` (required by the in-pod Docker daemon) and receives its JIT runner config through the `RUNNER_JITCONFIG` environment variable.

Only one scheduler at a time may run `sync_workers_state`: each invocation holds `LOCK TABLE workers IN EXCLUSIVE MODE` for its duration, using a thread-local connection pin (`db.hold_connection`) so all `mark_worker_*` calls share the locked transaction. If a second scheduler container is deployed, it blocks on the lock until the first commits.
...
Health checks for stuck runners run inside `sync_workers_state` and, rather than deleting the pod directly, *kill* it by patching `spec.activeDeadlineSeconds = 1`. The kubelet then transitions the pod to `Failed` (reason `DeadlineExceeded`) so it enters the normal 6-hour grace-and-delete flow. Logs and events remain inspectable:

- **RUNNER_NEVER_REGISTERED**: pod has been Running for more than `RUNNER_REGISTRATION_TIMEOUT_SECONDS` (120s) but the runner never appeared in the GitHub API. Worker is marked `failed` with full diagnostics in `failure_info`, then the pod is killed so its slot frees up for a retry.
...
Both health checks first attempt to delete the runner from GitHub if one is registered under that name. If GitHub refuses (e.g. `422 "Runner is busy"`), `sync_workers_state` aborts the cleanup for that worker. GitHub believes a job is actually running, so the worker is left alone and retried next cycle.
...
When `gh.authenticate_app()` returns 404 in the scheduler, the matching job is marked `failed` with `installation not found`, but the *cause* of the 404 is invisible. The user may have uninstalled the app, suspended it, removed access to a specific repo, renamed their org/user account, or installed the wrong app variant on the wrong account type. Without captured history, users cannot be told why their jobs stopped getting picked up.

`installation_events` is an append-only table that records, in chronological order:

- Every webhook delivery the app receives (`installation`, `installation_repositories`, `installation_target`, `workflow_job`, `ping`, plus a row for any unhandled `X-GitHub-Event` not modelled).
- Every scheduler `gh.authenticate_app()` failure (`auth_attempt.404`, `auth_attempt.other_error`).

Each row carries the full payload as `JSONB`, plus filter/index keys (`installation_id`, `app_id`, `entity_type`, `entity_id`, `entity_name`) and a free-form `outcome` string. The `WebhookOutcome` type in [`internal/contract.go`](internal/contract.go) is the canonical list of outcome values; the column itself is `TEXT` so new outcomes do not require schema migrations. `entity_id` is the GitHub `account.id`, which is stable across renames and reinstalls: uninstalling and reinstalling the app produces a new `installation_id` but keeps the same `entity_id`.

The webhook handler writes the `jobs` side-effect (`add_job`, `mark_job_running`, `mark_job_completed`) and the `installation_events` row in **separate transactions**. If the log write fails the side-effect has already committed; the handler returns 500 and GitHub redelivers. Re-deliveries converge: `add_job` uses `ON CONFLICT (job_id) DO NOTHING`, the worker-status updates are no-ops on a second run, and the log table has no UNIQUE constraint on payload so a duplicate log row is acceptable (the trace endpoints can dedupe by `delivery_id` from the JSONB payload when needed).

The scheduler's `ghAuthenticate` wrapper ([`cmd/scheduler/gh_auth.go`](cmd/scheduler/gh_auth.go)) only records failures: the underlying `AuthenticateApp` is TTL-cached, so success is the hot path and would drown the log. Failures are not cached, so transient errors do not poison subsequent calls.
...
The `/trace/*` endpoints on `ghfe` return events as JSON. Authentication is a simple `Authorization: Bearer $TRACE_API_SECRET` check (gates casual access only, not designed as a security boundary).
...
Per-entity configuration is defined in `EntityConfigs` in [`internal/constants.go`](internal/constants.go), keyed by entity ID (org ID or user ID):
...
| File | Purpose |
|------|---------|
| [`cmd/ghfe/`](cmd/ghfe/) | Webhook handler: validates requests, writes to PostgreSQL, serves `/setup/*` and `/trace/*` |
| [`cmd/scheduler/`](cmd/scheduler/) | Scheduler: GH reconciliation, demand matching, cleanup, worker status sync; serves `/usage`, `/history`, `/jobs`, `/workers` |
| [`internal/constants.go`](internal/constants.go) | Environment configuration, `EntityConfigs`, timeouts, image tags |
| [`internal/contract.go`](internal/contract.go) | Shared types, `WebhookOutcome` enum, DB/GitHub/Kube interfaces |
| [`internal/db.go`](internal/db.go) | PostgreSQL operations (pgx) |
| [`internal/github.go`](internal/github.go) | GitHub App auth + REST client |
| [`internal/k8s.go`](internal/k8s.go) | Kubernetes pod provisioning, deletion, capacity checks, failure-info collection |
| [`internal/testutil/`](internal/testutil/) | In-memory fakes shared by `cmd/` tests |
| [`Dockerfile`](Dockerfile) | Multi-stage build producing the `ghfe` and `scheduler` images |
| [`serverless.yml`](serverless.yml) | Scaleway Serverless deployment manifest |
| [`../scripts/trace_installation.py`](../scripts/trace_installation.py) | CLI client for the `/trace/*` endpoints: chronological table + diagnosis hints |
...
Tests run against in-memory fakes for PostgreSQL, the GitHub API, and the Kubernetes API. No live services are required.

## Deployment

Deployment is handled automatically by [`../.github/workflows/deploy-container.yml`](../.github/workflows/deploy-container.yml).
...
Production and staging each have their own k8s cluster on Scaleway, managed via scripts in [`../scripts/`](../scripts/).
...
ssh root@$(scw instance server list zone=fr-par-2 project-id=03a2e06e-e7c1-45a6-9f05-775d813c2e28 -o json | jq -r '.[] | select(.name == "<control-plane-name>") | .public_ip.address') cat /etc/kubernetes/kubeconfig-gh-app.conf      | gh secret set K8S_KUBECONFIG --repo riseproject-dev/riscv-runner --env prod
ssh root@$(scw instance server list zone=fr-par-2 project-id=03a2e06e-e7c1-45a6-9f05-775d813c2e28 -o json | jq -r '.[] | select(.name == "<control-plane-name>") | .public_ip.address') cat /etc/kubernetes/kubeconfig-gh-deploy.conf   | gh secret set K8S_KUBECONFIG --repo riseproject-dev/riscv-runner --env prod
```
